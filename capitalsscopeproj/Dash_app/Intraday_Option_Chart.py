...
#from strategy.kite_trade import *
...
eq_bhav10 = (f"SELECT distinct * FROM capitalsscope.live_option_chain_nifty;")

symbolll = pd.read_sql(sql=eq_bhav10, con=engine)

symbolll['Date1'] = pd.to_datetime(symbolll['Date'], format="%d-%m-%Y").dt.date
intraday = pd.unique((symbolll["Date1"]).values.tolist())[-1]
...
def indexoption(index):
    ...
    script_codee = pd.read_csv(script_code_url,low_memory=False)
...
data_pcr = pcr_data(day,index)
# print(data_pcr.tail(3))

data_eq3 = data_eq[data_eq.DATE == day]
...
dgf = data_eq[(data_eq['DATE'] == day) &  (data_eq['Time'] == time1)]
...
lengt = round((dgf.shape[0])/2)
re1 = dgf['StrikeRate'].iloc[lengt]
re2 = dgf['StrikeRate'].iloc[lengt+1]
diff = (re2-re1)
...
colour_CE_Chg_OI = data_bars2(data_eq, 'CE_Chg_OI')
colour_PE_Chg_OI = data_bars2(data_eq, 'PE_Chg_OI')
...
app = DjangoDash('Intraday_Option_Chart',
                 external_stylesheets=external_stylesheets)
data_eq['id'] = data_eq.index

app.layout = html.Div([
    dcc.Interval(id='graph_int',interval=1*30000),
    ...
    html.Div([dash_table.DataTable(id='my_table',
                                    ...
                                    style_data_conditional=(
                                        ...
                                        [{'if':{'filter_query':'{StrikeRate} > 100','column_id':'StrikeRate'},'backgroundColor':'#c98122','color':'white'}, 
                                         #{'if': {'filter_query': '{{StrikeRate}} = {}'.format((len(data_eq['StrikeRate'])+1)/2),'column_id': 'StrikeRate'},'backgroundColor': 'hotpink','color': 'white'},
                                         #{'if': {'filter_query': '{id} = {}'.format((len(data_eq['StrikeRate'])+1)/2), 'column_id': 'StrikeRate'},'backgroundColor': 'RebeccaPurple'},
                                         {'if': {'row_index': 9,'column_id': 'StrikeRate'},'backgroundColor': 'hotpink','color': 'white'},
...
def update_dropdown_options(Date_drop,graph_int):
...
@app.callback(
    ...
    Input('graph_int', 'n_intervals')])
def get_date_value(time_v,graph_int):
...
def update_dropdown_options(Date_drop,Time_drop,optionns,row_drop,graph_int):
...
def pcr(Date_drop,Time_drop,optionns,row_drop,graph_int):

    
    print(Date_drop,Time_drop,optionns,row_drop)
    Date_dropp = pd.to_datetime(Date_drop)
    dffg = pcr_data(Date_dropp,optionns)
...
def build_graph1(Date_drop,Drop_11,Drop_12, Drop_13, Drop_14,Drop_15, Drop_16,graph_int):
    date_v = pd.to_datetime(Date_drop)
    dfff = data_eq[data_eq.DATE == date_v.date()] 
...
def build_graph2(Date_drop,Drop_21,Drop_22, Drop_23, Drop_24,Drop_25, Drop_26,graph_int):
    date_v = pd.to_datetime(Date_drop)
    dfff = data_eq[data_eq.DATE == date_v.date()]   
...
def build_graph3(Date_drop,Drop_31,Drop_32, Drop_33, Drop_34,Drop_35, Drop_36,graph_int):
    date_v = pd.to_datetime(Date_drop)
    dfff = data_eq[data_eq.DATE == date_v.date()] 
...
def build_graph4(Date_drop,Drop_41,Drop_42, Drop_43, Drop_44,Drop_45, Drop_46,graph_int):
    date_v = pd.to_datetime(Date_drop)
    dfff = data_eq[data_eq.DATE == date_v.date()]   
