...
#                     'NIFTY PHARMA', 'NIFTY PSU BANK', 'NIFTY REALTY', 'NIFTY PRIVAT BANK',
...
#                     'NIFTY MNC', 'NIFTY GROWTH SECTORS 15', 'NIFTY PSE', 'NIFTY SERVICE SECTOR',
#                     'NIFTY100 LIQUID 15', 'NIFTY MIDCAP LIQUID 15']

live_market_keys = ['NIFTY 50','NIFTY NEXT 50','NIFTY 100','NIFTY 200','NIFTY 500','NIFTY MIDCAP 50',
                    'NIFTY MIDCAP 100','NIFTY SMALLCAP 100','NIFTY MIDCAP 150','NIFTY SMALLCAP 50',
                    'NIFTY SMALLCAP 250','NIFTY MIDSMALLCAP 400','NIFTY500 MULTICAP 50:25:25',
                    'NIFTY LARGEMIDCAP 250','NIFTY MIDCAP SELECT','NIFTY TOTAL MARKET','NIFTY MICROCAP 250',
                    'NIFTY BANK','NIFTY AUTO','NIFTY FINANCIAL SERVICES',
                    'NIFTY FINANCIAL SERVICES 25/50','NIFTY FMCG','NIFTY IT','NIFTY MEDIA','NIFTY METAL',
                    'NIFTY PHARMA','NIFTY PSU BANK','NIFTY PRIVATE BANK','NIFTY REALTY','NIFTY HEALTHCARE INDEX',
                    'NIFTY CONSUMER DURABLES','NIFTY OIL & GAS','NIFTY DIVIDEND OPPORTUNITIES 50',
                    'NIFTY GROWTH SECTORS 15','NIFTY100 QUALITY 30','NIFTY50 VALUE 20',
                    'NIFTY ALPHA 50','NIFTY50 EQUAL WEIGHT','NIFTY100 EQUAL WEIGHT',
                    'NIFTY100 LOW VOLATILITY 30','NIFTY200 QUALITY 30','NIFTY ALPHA LOW-VOLATILITY 30',
                    'NIFTY200 MOMENTUM 30','NIFTY MIDCAP150 QUALITY 50','NIFTY COMMODITIES',
                    'NIFTY INDIA CONSUMPTION','NIFTY CPSE','NIFTY ENERGY','NIFTY INFRASTRUCTURE',
                    'NIFTY100 LIQUID 15','NIFTY MIDCAP LIQUID 15','NIFTY MNC','NIFTY PSE','NIFTY SERVICES SECTOR',
...
if not os.path.exists("Break_vol_pri_intr.xlsx"):
    try:
        wb = xw.Book()
        wb.sheets.add("optionchain")
        wb.save("Break_vol_pri_intr.xlsx")
        wb.close()
    except Exception as e:
        print(f"Error : {e}")
        sys.exit()
wb = xw.Book('Break_vol_pri_intr.xlsx')
...
while True:
    ...
    index_frame_one.sort_values(['change'], ascending=[True], inplace=True)
    index_frame_one = index_frame_one[['Index','open','dayHigh','dayLow','lastPrice','previousClose','change','pChange','totalTradedVolume','totalTradedValue','lastUpdateTime','Post','Negat','Neutral','Total','Per','yearHigh','yearLow','nearWKH','nearWKL']]
    st1.range("a:n").value = None
    st1.range("a1").options(index=False).value = index_frame_one

    index_frame_two = index_frame_one
    index_frame_two.sort_values(['Per'], ascending=[True], inplace=True)
    index_frame_two = index_frame_two[['Index','open','dayHigh','dayLow','lastPrice','previousClose','change','pChange','totalTradedVolume','totalTradedValue','lastUpdateTime','Post','Negat','Neutral','Total','Per','yearHigh','yearLow','nearWKH','nearWKL']]
    st2.range("a:n").value = None
    st2.range("a1").options(index=False).value = index_frame_two

    index_frame.sort_values(['Index','change'], ascending=[True,True], inplace=True)
    index_frame = index_frame[['Index','identifier','Weitage','open','dayHigh','dayLow','lastPrice','previousClose','change','pChange','totalTradedVolume','totalTradedValue','lastUpdateTime']]
    ...
    st1.range("p1").options(index=False).value = "Today's rangs is"
    st1.range("p3").options(index=False).value = nifty_range
    st1.range("p2").options(index=False).value = bank_nifty_range
    ...
    print("SectorWise Data Download")
    ...
    def five_df_new1_func():  
        ...
        for a in script_list:
            try:
                print(a)
                dfg = client.historical_data('N', 'C', a, '5m', current_trading_day, current_trading_day)
                dfg['Scripcode'] = a
                # dfg['Date'] = current_trading_day
                # dfg["EMA_200"] = np.round((pta.sma(dfg["Close"], length=200,offset=0)),2)
                # dfg1 = dfg.iloc[::-1]
                dfg["SMA_10"] = np.round((pta.sma(dfg["Close"], length=10,offset=0)),2)
                dfg["EMA_13"] = np.round((pta.ema(dfg["Close"], length=13,offset=0)),2)
    ...
    # data_eq1 = pd.concat([delv_data, five_df_new1])

    data_eq1 = five_df_new1
    # data_eq1.bfill(axis ='rows')
    data_eq1.sort_values(['Name'], ascending=[True,], inplace=True)
    # data_eq1.bfill(axis ='rows')
    ...
    def final_data_func():
        ...
        for d in stk_list:
            ...
            eq_data1.sort_values(['Name'], ascending=[True,], inplace=True)
            eq_data1['Time'] = datetime.now()
            
            # eq_data1['200+'] = ((eq_data1['EMA_200']*1)/100)+(eq_data1['EMA_200'])
            # eq_data1['200-'] = (eq_data1['EMA_200'])-((eq_data1['EMA_200']*1)/100)
            
            # eq_data1['Delv_Chg'] = round(((eq_data1['Deliv_qty'] * 100) / (eq_data1['Deliv_qty'].shift(-1)) - 100), 2).fillna(0)      
            eq_data1['Ema_break'] = np.where(eq_data1['SMA_10'] > eq_data1["EMA_13"],"Ema_Up","Ema_Down")
            ...
            # eq_data1['OI_Vol_Pri_break'] = np.where((eq_data1['OI_break'].shift(-1) == "OI_brk") &
            #                                         (eq_data1['Vol_Price_break'] == "Vol_Pri_break"), "OI_Vol_Pri_break", "")
        ...
        eq_data_pd.sort_values(['Name'], ascending=[True], inplace=True)
        return eq_data_pd
    ...
    # # print(eq_data_pd.tail(2))
    # eq_data_pd['concate'] = eq_data_pd['Deliv_break'] + eq_data_pd['Price_break'] + eq_data_pd['Vol_break'] + eq_data_pd['Del_Vol_Pri_break'] +eq_data_pd['OI_Vol_Pri_break']
    # # print(eq_data_pd.tail(2))
    ...
    orders_select1 = eq_data_pd[(eq_data_pd["Vol_break"] == "Vol_brk") & (eq_data_pd["Price_break"] != "") & (eq_data_pd["Buy/Sell"] != "")]
    ...
    orders_select1 = orders_select1[['Name','Buy/Sell','Scripcode','Datetime','Open','High','Low','Close','Price_Chg','Vol_Chg','Price_break','O=H=L']]
    strategy1.range("a:r").value = None
    strategy1.range("a1").options(index=False).value = orders_select1

    orders_select2 = eq_data_pd[(eq_data_pd["Vol_break"] == "Vol_brk") & (eq_data_pd["Price_break"] != "") & (eq_data_pd["Buy/Sell"] != "") & (eq_data_pd["O=H=L"] != "") & (eq_data_pd["Close"] < 300)]
    orders_select2["Watchlist"] = "N" + ":" + "C" + ":" + orders_select2["Name"]
    orders_select2 = orders_select2[['Name','Buy/Sell','Scripcode','Datetime','Open','High','Low','Close','Price_Chg','Vol_Chg','Price_break','O=H=L']]
    strategy2.range("a:r").value = None
    strategy2.range("a1").options(index=False).value = orders_select2

    orders_select3 = eq_data_pd[(eq_data_pd["Vol_break"] == "Vol_brk") & (eq_data_pd["Price_break"] != "") & (eq_data_pd["Buy/Sell"] != "") & (eq_data_pd["Close"] < 300)]
    orders_select3["Watchlist"] = "N" + ":" + "C" + ":" + orders_select3["Name"]
    orders_select3 = orders_select3[['Name','Buy/Sell','Scripcode','Datetime','Open','High','Low','Close','Price_Chg','Vol_Chg','Price_break','O=H=L']]
