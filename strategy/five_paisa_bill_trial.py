...
path = "E:/STOCK/five_paisa_bill1/2023-04-21.pdf"
...
for file in files_dir:
    ...
    dff = tabula.io.read_pdf(file,pages="all",password="CIWPB5219G")
    tot_df = len(dff)
    print(tot_df)
    ...
    STT = summary[summary[summary.columns[3]] == "SELL"]
    STT_Total = np.round(((STT[STT.columns[9]]).sum()),1)
    Brok = summary[summary[summary.columns[6]] != 0]
    Brok_Total = np.round(abs(Brok[Brok.columns[10]].sum()),2)
    Exch_Trans_Chg = summary[summary[summary.columns[3]] == "SELL"]
    Exch_Trans_Chg_Total = np.round((((Exch_Trans_Chg[Exch_Trans_Chg.columns[10]])*Ex_Tr_Ch).sum()),2).item()
    SEBI_Turn_chg = summary[summary[summary.columns[3]] == "SELL"]
    SEBI_Turn_Total = np.round(((SEBI_Turn_chg[SEBI_Turn_chg.columns[7]].count())*SEBI_Turn),2)
    Taxable_value = np.round((Brok_Total+Exch_Trans_Chg_Total+SEBI_Turn_Total),2)
    IGST_Total = np.round((Taxable_value*IGST),2)
    Stamp_Dut = summary[summary[summary.columns[3]] == "SELL"]
    Stamp_Dut1 = np.round(((Stamp_Dut[Stamp_Dut.columns[10]]).sum()),1)
    Net_Total = (summary[summary.columns[10]]).sum()
...
if not os.path.exists("five_paisa_bill_trial.xlsx"):
    try:
        wb = xw.Book()
        wb.save("five_paisa_bill_trial.xlsx")
        wb.close()
    except Exception as e:
        print(f"Error : {e}")
        sys.exit()
wb = xw.Book('five_paisa_bill_trial.xlsx')
