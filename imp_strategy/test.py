...
# username1 = str(username)
# client = credentials(username1)
users = ["MUKESH","HARESH"]#,"ASHWIN","ALPESH"]
credi_har = None
credi_muk = None
...
cred = [credi_muk]#,credi_har,credi_ash,credi_alp]
print(cred)
for credi in cred:
    ...
    if dfg4.empty:
        ...
        if orders.upper() == "YES" or orders.upper() == "":  
            for credi in cred:
                #postt = pd.DataFrame(credi.margin())['Ledgerbalance'][0]
                #print(f"Ledger Balance is : {postt}") 
                order = credi.place_order(OrderType=order_side,Exchange='N',ExchangeType='D', ScripCode = scrip_code, Qty=quantity*4,Price=price_of_stock, IsIntraday=True)# IsStopLossOrder=True, StopLossPrice=Buy_Stop_Loss)
...
def data_download(stk_nm,vol_pr,rsi_up_lvll,rsi_dn_lvll):
    df = pd.DataFrame(credi_mukesh.historical_data(260105, second_last_trading_day, to_d, "5minute", continuous=False, oi=True))
