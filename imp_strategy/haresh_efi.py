...
sam_21_slop = 1
dema_21_slope = 3
efi_slop = 2 #(IN MILLIONS)
slllll = -400
tgtttt = 1000
vol_opt = 1 #(IN MILLIONS)
vol_multiply = 3 #(TIMES X)
...
st.range("af1").value = "ON"
st.range("ag1").value = "Tele_Msg"
st.range("ah1").value = "OFF"

st.range("ae2").value = "StopLoss"
st.range("af2").value = slllll
st.range("ag2").value = "Target"
st.range("ah2").value = tgtttt

st.range("ae3").value = "Trailing"
st.range("af3").value = "OFF"
st.range("ag3").value = "Exit"
st.range("ah3").value = "OFF"

st.range("ae4").value = "ADX"
st.range("af4").value = "ON"

def calculate_elder_force_index(data, short_period=2, long_period=13):
    # Calculate the daily Force Index: (Close - Close_prev) * Volume
    data['Price_Change'] = data['Close'].diff()
    data['Force_Index'] = data['Price_Change'] * data['Volume']

    # Calculate the EMA of the Force Index for short and long periods
    data['EFI_Short'] = data['Force_Index'].ewm(span=short_period, adjust=False).mean()
    data['EFI_Long'] = data['Force_Index'].ewm(span=long_period, adjust=False).mean()

    return data[['Close', 'Volume', 'Force_Index', 'EFI_Short', 'EFI_Long']]
...
def order_execution(df,list_append_on,list_to_append,telegram_msg,orders,CALL_PUT,BUY_EXIT,order_side,scrip_code,qtyy,namee,stk_name):
    ...
    if dfg4.empty:
        ...
        if order_side == "B":
            price_of_stoc = float(dfg4['Close'].iloc[0])
        ...
        if telegram_msg.upper() == "ON" or telegram_msg.upper() == "":
...
def data_download(stk_nm,adx_chk,vol_pr,rsi_up_lvll,rsi_dn_lvll):
    ...
    # df['Call'] = np.where((df['Adx_ok'] == "ok") & (df['SMA_21_ok'] == "up_ok") & (df['DEMA_21_ok'] == "up_ok") & (df['CROSS'] == "up_ok"),"Call_Buy","Call_Exit")
    # df['Put'] = np.where((df['Adx_ok'] == "ok") & (df['SMA_21_ok'] == "dn_ok") & (df['DEMA_21_ok'] == "dn_ok") & (df['CROSS'] == "dn_ok"),"Put_Buy","Put_Exit")
    
    if adx_chk.upper() == "ON":
        df['Call'] = np.where((df['Adx_ok'] == "ok") & (df['SMA_21_ok'] == "up_ok") & (df['DEMA_21_ok'] == "up_ok") ,"Call_Buy","Call_Exit")
        df['Put'] = np.where((df['Adx_ok'] == "ok") & (df['SMA_21_ok'] == "dn_ok") & (df['DEMA_21_ok'] == "dn_ok"),"Put_Buy","Put_Exit")
    if adx_chk.upper() == "OFF":
        df['Call'] = np.where((df['SMA_21_ok'] == "up_ok") & (df['DEMA_21_ok'] == "up_ok") ,"Call_Buy","Call_Exit")
        df['Put'] = np.where((df['SMA_21_ok'] == "dn_ok") & (df['DEMA_21_ok'] == "dn_ok"),"Put_Buy","Put_Exit")
    ...
    #df["Time"] = df['Datetime'].apply(lambda x: datetime.strptime(x,'%H:%M')).
...
while True:
    ...
    sllll,tgttt = st.range("af2").value,st.range("ah2").value
    trailing,exitt,adx_check = st.range("af3").value,st.range("ah3").value,st.range("af4").value
    ...
    if exitt is None:
        exitt = "OFF"
    if adx_check is None:
        adx_check = "ON"

    #orders = "OFF"
    # print(buy_order_list_dummy)
    # print(sell_order_list_dummy)
    print("Orders is: "+str(orders))
    print("Telegram Msg is: "+str(telegram_msg))
    print("Stop Loss is: "+str(sllll))
    print("Target is: "+str(tgttt))
    print("Trailing is: "+str(trailing))
    print("Exit is: "+str(exitt))
    print("ADX is: "+str(adx_check))
    ...
    try:      
        for sc in stk_list:
            dfg1 = data_download(sc,adx_check,Vol_per,UP_Rsi_lvl,DN_Rsi_lvl) 
            ...
            dfg1112 = dfg111.tail(20)
            five_df1 = pd.concat([dfg1112, five_df1]) 
    

            Call_by_df = dfg1[(dfg1["Call"] == "Call_Buy")]
            # Call_by_df['Date_Dif'] = abs((Call_by_df["Datetime"] - Call_by_df["Datetime"].shift(1)).dt.total_seconds() / 60)
            ...
            Call_by_df11 = Call_by_df[(Call_by_df["Date"] == current_trading_day.date())]        
            five_df2 = pd.concat([Call_by_df11, five_df2])
            
            Call_by_df2 = Call_by_df[(Call_by_df["Date"] == current_trading_day.date()) & (Call_by_df["Minutes"] < 5 )]   
            ...
            if Call_by_df2.empty:
                ...
                Call_by_Closee = (float(Call_by_ord['Close'].iloc[0] ))
                ...
                Call_by_Scripcodee = int(float(Call_by_ord6['ScripCode'].iloc[0]))
                Call_by_Qtyy = int(Call_by_ord6['LotSize'].iloc[0])
                ...
                if not Call_by_ord6.empty:                     
                    dfg1_Call_by = credi_har.historical_data('N', 'D', Call_by_Scripcodee, '5m', second_last_trading_day,current_trading_day)
                    dfg1_efi = calculate_elder_force_index(dfg1_Call_by)
                    dfg1_Call_by['EFI'] = np.round((dfg1_efi[dfg1_efi.columns[4]]),2)
                    dfg1_Call_by['Cand_Coll'] = np.where(dfg1_Call_by['Close'] > dfg1_Call_by['Open'],"Green",np.where(dfg1_Call_by['Close'] < dfg1_Call_by['Open'],"Red","") ) 
                    dfg1_Call_by['Vol_ok'] = np.where((dfg1_Call_by['Volume'] > vol_opt*100000) & (dfg1_Call_by['Cand_Coll'] == "Green"),"V_ok","")
                    dfg1_Call_by = dfg1_Call_by[['Datetime','Open','High', 'Low', 'Close', 'Volume','Price_Change','EFI','Vol_ok']]
                    dfg1_Call_by['Efi_diff'] = dfg1_Call_by['EFI'] - dfg1_Call_by['EFI'].shift(1)
                    dfg1_Call_by['Efi_ok'] = np.where((dfg1_Call_by['Vol_ok'] == 'V_ok') & (dfg1_Call_by['Efi_diff'] > (efi_slop*100000)) & (dfg1_Call_by['EFI'] > (dfg1_Call_by['Volume']*vol_multiply)),'ok','')
                    print(dfg1_Call_by.tail(1))
                    dfg1_Call_by1 = dfg1_Call_by.tail(1)
                    dfg1_Call_by2 = dfg1_Call_by1[(dfg1_Call_by1["Efi_ok"] == "ok")]
                    # dfg1_Call_by2 = dfg1_Call_by
                    if dfg1_Call_by2.empty:
                        print("No Call Buy Option Position Activate")
                    else:
                        if Call_by_time in buy_order_list_dummy: 
                            print(str(stk_name)+" Call Option is Already Buy")
                            print("----------------------------------------")
                        else:
                            print("Call Option Buy")                        
            ...
            Put_by_df = dfg1[(dfg1["Put"] == "Put_Buy")]
            # Put_by_df['Date_Dif'] = abs((Put_by_df["Datetime"] - Put_by_df["Datetime"].shift(1)).dt.total_seconds() / 60)
            ...
            Put_by_df11 = Put_by_df[(Put_by_df["Date"] == current_trading_day.date())]  
            five_df3 = pd.concat([Put_by_df11, five_df3]) 

            Put_by_df2 = Put_by_df[(Put_by_df["Date"] == current_trading_day.date()) & (Put_by_df["Minutes"] < 5 )]          
            if Put_by_df2.empty:
                #pass
                print("Put Buy DF Empty")
            else:   
                Put_by_ord = Put_by_df2.tail(1)
                Put_by_Closee = (float(Put_by_ord['Close'].iloc[0]))
                ...
                Put_by_Scripcodee = int(float(Put_by_ord6['ScripCode'].iloc[0]))
                Put_by_Qtyy = int(np.unique(Put_by_ord6['LotSize'].iloc[0]))
                
                print(Put_by_Scripcodee,Put_by_Qtyy,Put_by_time)
                #order_execution(df,list_append_on,list_to_append,telegram_msg,orders,CALL_PUT,BUY_EXIT,order_side,scrip_code,qtyy,Buy_At,namee)
                
                if not Put_by_ord6.empty:
                    dfg1_Put_by = credi_har.historical_data('N', 'D', Put_by_Scripcodee, '5m', second_last_trading_day,current_trading_day)
                    # dfg1_Put_by['Cand_Coll'] = np.where(dfg1_Put_by['Close'] > dfg1_Put_by['Open'],"Green",np.where(dfg1_Put_by['Close'] < dfg1_Put_by['Open'],"Red","") ) 
                    # dfg1_Put_by['Vol_ok'] = np.where((dfg1_Put_by['Volume'] > vol_opt*100000) & (dfg1_Put_by['Cand_Coll'] == "Green"),"V_ok","")
                    dfg1_efi = calculate_elder_force_index(dfg1_Put_by)
                    dfg1_Put_by['EFI'] = np.round((dfg1_efi[dfg1_efi.columns[4]]),2)
                    dfg1_Put_by['Cand_Coll'] = np.where(dfg1_Put_by['Close'] > dfg1_Put_by['Open'],"Green",np.where(dfg1_Put_by['Close'] < dfg1_Put_by['Open'],"Red","") ) 
                    dfg1_Put_by['Vol_ok'] = np.where((dfg1_Put_by['Volume'] > vol_opt*100000) & (dfg1_Put_by['Cand_Coll'] == "Green"),"V_ok","")
                    dfg1_Put_by = dfg1_Put_by[['Datetime','Open','High', 'Low', 'Close', 'Volume','Price_Change','EFI','Vol_ok']]
                    dfg1_Put_by['Efi_diff'] = dfg1_Put_by['EFI'] - dfg1_Put_by['EFI'].shift(1)
                    dfg1_Put_by['Efi_ok'] = np.where((dfg1_Put_by['Vol_ok'] == 'V_ok') & (dfg1_Put_by['Efi_diff'] > (efi_slop*100000)) & (dfg1_Put_by['EFI'] > (dfg1_Put_by['Volume']*vol_multiply)),'ok','')
                    print(dfg1_Put_by.tail(1))
                    ...
                    dfg1_Put_by2 = dfg1_Put_by1[(dfg1_Put_by1["Efi_ok"] == "ok")]
                    # dfg1_Put_by2 = dfg1_Put_by
                    if dfg1_Put_by2.empty:
                        print("No Put Buy Option Position Activate")
                    else:
                        if Put_by_time in buy_order_list_dummy: 
                            print(str(stk_name)+" Put Option is Already Buy")
                            print("----------------------------------------")
                        else:
                            print("Put Option Buy")                        
                ...
                if posit.empty:
                    ...
                    if exitt.upper() == "ON":
                        ...
                        if pl < slll or pl > tgtt:
                            ...
                            print("StopLoss is Greater than -400")
                            print("Fixed Sell order Executed")
                        else:
                            if trailing.upper() == "ON":
                                    ...
                                    if order_dff.empty:
                                        ...
                                        try: 
                                            ...
                                            for ordd in order_dff_Scpt:
                                                ...
                                                print("Traling Sell order Executed") 
                                        except Exception as e:
                                            print(e)
    except Exception as e:
        print(e) 
