...
def data_download(stk_nm,start_dt,end_dt,vol_pr):
        print(stk_nm)
        scpt0 = exc_fut[exc_fut['Root'] == stk_nm]
        scpt1 = scpt0[(scpt0['Expiry'].apply(pd.to_datetime) >= end_dt)]
        aaa = int(scpt1['Scripcode'])

        dfg1 = client.historical_data('N', 'D', aaa, '1m',start_dt,end_dt) 
        dfg1['Scripcode'] = aaa
        print(dfg1.head(1))
...
#stop_thread = False
thread = threading.Thread(target=data_download,args=('BSOFT',last_trading_day,current_trading_day,15))
thread.start()
...
#             # thread.start()
#             dfg1 = data_download(sc,last_trading_day,current_trading_day,Vol_per)   
#             #print(dfg1.head(1))        
...
#             aaa = (np.unique([str(i) for i in dfg1['Name']])).tolist()[0]
