...
if not os.path.exists("expiry_special_old.xlsx"):
    try:
        wb = xw.Book()
        wb.sheets.add("optionchain")
        wb.save("expiry_special_old.xlsx")
        wb.close()
    except Exception as e:
        print(f"Error : {e}")
        sys.exit()
wb = xw.Book('expiry_special_old.xlsx')
...
script_code_5paisa2 = script_code_5paisa1[(script_code_5paisa1["Exch"] == "N") & (script_code_5paisa1['ExchType'].isin(['D']))]
exc_new2 = script_code_5paisa2[(script_code_5paisa2['Expiry'].apply(pd.to_datetime) == current_trading_day)]
exc_new3 = exc_new2[(exc_new2["ScripType"] == "XX")]
flt_exc.range("a5").options(index=False).value = exc_new2
flt_exc.range("a1").options(index=False).value = exc_new3

segment1 = "nse_eq"
script_code_5paisa_url3 = f"https://Openapi.5paisa.com/VendorsAPI/Service1.svc/ScripMaster/segment/{segment1}"
script_code_5paisa3 = pd.read_csv(script_code_5paisa_url3,low_memory=False)
script_code_5paisa4 = script_code_5paisa3[(script_code_5paisa3["Exch"] == "N") & (script_code_5paisa3["ExchType"] == "C") & (script_code_5paisa3["Series"] == "EQ") & (script_code_5paisa3["ScripType"] == "EQ")]
flt_exc.range("a500").options(index=False).value = script_code_5paisa4

neww_exch = pd.merge(exc_new3, script_code_5paisa4, on=['SymbolRoot'], how='inner') 
neww_exch.rename(columns={'ScripCode_y': 'ScripCode','Name_y': 'Name'}, inplace=True)
print(neww_exch)

stk_list = (np.unique(neww_exch['ScripCode']).tolist())
print(stk_list)
...
def data_download(stk_nm,vol_pr,rsi_up_lvll,rsi_dn_lvll):
    ...
    print(df.head(1))
    df['ScripCode'] = int(stk_nm)

    df = pd.merge(neww_exch, df, on=['ScripCode'], how='inner') 
    df = df[['ScripCode','Name','Datetime','Open','High','Low','Close','Volume']]
    print(df.head(1))
